...
2. 点击开始前，前端展示首点、点位顺序、当前地图、点位合法性和开跑前记录命令。
...
6. dwell 结束后切到下一个点；如果任务结束，状态、timeline、`last_result` 一致。
7. 前端、Nav2 状态、`last_result.runtime_snapshot` 能解释任务到底是运行、等待、dwell、卡滞、超时、路径不匹配还是完成。
...
dashboard.html/css/js
  ...
  -> web_dashboard_node.py 更新时间线、状态和 last_result
  -> dashboard.js 实时显示
```
...
5. `active_waypoint`、`last_result.runtime_snapshot`、前端 `task_execution_evidence` 和 watcher 的 `floor_goal_published` / `floor_goal_publishes` 列共同用于复盘“发没发、接没接、路对不对、卡在哪”。

如果任务页显示导航中，但没有 `floor_goal_published`，断点在 Web 发布目标前；如果已经发布但 Nav2 没有 accepted/feedback，断点在 `/m20pro/floor_goal` 到 floor manager / Nav2；如果 Nav2 接收了但 `path_goal_error_m` 大或 `plan_goal_verified=false`，断点是路径目标不匹配；如果路径正确但机器人不动或漂移，再看定位、避障、runtime guard 和底盘状态。
...
前端负责展示和用户确认，不负责决定任务是否安全。任务能否开始必须由后端 readiness 和 contract 决定。
...
- lidar relay 诊断；
- runtime guard；
- 最终 `last_result.runtime_snapshot`。

现场复盘优先看这里，而不是只看前端一句“失败”。
...
- 前端定位成功/失败的判据；
- readiness 中 `localization_ok` 的证据链。

A 不应该直接改 active task 状态机，也不应该绕过任务 readiness 去允许开跑。如果需要新增定位字段，应先进入 readiness 或 snapshot contract，再由前端展示。
...
- 到点后进入 dwell；
- dwell 后切到下一个点；
- `last_result`、timeline、Nav2 状态一致。

拿到这组证据前，不能把单层导航目标标记为完成。

## 当前阻断状态

目标模式不能继续上车推进时，先区分前端问题和底层门槛。当前已知的底层门槛是：

- 电量低于目标模式门槛：以 `./scripts/104_goal_mode_battery_gate.py --url http://10.21.31.104:8080` 的最新只读输出为准；低于 `25%` 时停止目标模式实机推进。
- 感知链路仍有硬故障：`/api/state.perception_status.code=factory_lidar_points_publisher_missing`，表示原厂 `/LIDAR/POINTS` 当前没有 DDS publisher，relay、`/scan` 和避障输入不能视为真实可用。

在这两个门槛未恢复前，只允许做本地代码收敛、文档维护和离线 contract 测试；不部署、不构建 104、不重启服务、不调用重定位、不启动任务、不发布运动目标。
...
3. 重新读取 `/api/state`，确认：
   ...
   - `task_readiness.code=map_relocalization_required` 或更具体的阻断原因；
   - `perception_status` 明确给出当前感知状态。
4. 先处理 `factory_lidar_points_publisher_missing`，直到 `/LIDAR/POINTS -> lidar_relay -> /scan -> perception_status` 全链路恢复。若需要重启原厂 `rsdriver.service` / `hsLidar.service`，必须由现场明确授权。
5. 感知恢复后，按开发手册 2101 跑前端重定位。成功判据必须是 TCP `Type=2101`、`Command=1`、`ErrorCode=0`，并且前端显示定位已确认、任务页可启动。
6. 只在当前固定地图上重新标当前地图任务点，创建当前地图任务。不要复用旧地图任务。
7. 运动前先运行 ready-check 和 watcher。确认首点、顺序、地图、pose、scan、lidar、Nav2 plan endpoint 都一致。
8. 明确允许运动后，才从前端点击任务开始。验收只看真实闭环证据：首点、dwell、下一个点、失败/完成的 `last_result.runtime_snapshot` 和 timeline。
