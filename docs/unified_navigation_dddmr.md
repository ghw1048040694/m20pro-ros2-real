...
两节点的状态话题都持续发布组件身份和启用状态。Web 只接受两个组件同时新鲜、
就绪且不忙的状态，避免只启动执行器或只启动编排器后把连接边推进到等待超时。
连接边的唯一运行身份为 `request_id + route_id + plan_id + map_epoch`；epoch 在
Web 启动连接边前持久化预留，切层事务、terrain guard 和迟到回执都必须匹配这四个
字段。106 收到 malformed 或其他连接边的释放请求时保留当前 terrain 所有权，
不会被一条迟到消息清空。
