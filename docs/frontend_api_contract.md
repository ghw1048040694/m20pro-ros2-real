...
兼容投影，不能作为另一套任务顺序来源。每次开始任务时，后端都会用当前
点位顺序和当前有向路线重新生成并刷新该投影；路线调整后可以直接重新执行
原任务，不需要为了旧计划缓存删除并重建任务。
...
`navigation_plan` 是点位库和有向路线库的紧凑投影，不是第二份路线数据库。
历史任务第一次启动时会生成该投影；已有任务每次启动前也会从当前点位和路线
重新生成并保存投影。任务执行期间禁止修改点位和跨楼层路线，因此一次运行中的
计划不会被中途替换。
...
跨楼层任务还应同时读取 `floor_switch_transaction`。`SWITCHING_MAP` 表示 104 Nav2 地图与 106 原厂地图正在并行激活，`RELOCALIZING` 表示正在目标共享平台执行 2101，`COMMITTED` 表示可以继续目标层出口导航，`FAILED` 表示当次任务已停止。连接边处于活动状态时，入口和出口 Nav2 都由 `stair_executor` 独占；楼层上下文提前更新为目标层不会触发任务主循环并发下发后续任务点。执行器 disabled/busy 或入口/出口 Nav2 立即失败时，状态回执携带本次连接边身份并直接结束当次任务，不等待阶段总超时。
