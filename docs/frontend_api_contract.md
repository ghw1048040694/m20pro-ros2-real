...
- 前端用一个姿态按钮在起立/趴下之间切换，优先依据原厂 `/MOTION_STATE` 决定目标动作。起立、趴下和软急停都先发送零速度并锁定速度仲裁器；起立/趴下动作结束后必须重新申请人工接管。原厂动作分别对应 `Type=2/Command=22` 的 `MotionParam=1/4`，软急停为 `MotionParam=2`。
