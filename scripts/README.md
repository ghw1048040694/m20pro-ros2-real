...
schema v2 共 67 个现场参数，其中 `navigation` 28 个，覆盖速度/加速度、到点、卡住判定、DWB 采样、局部/全局代价地图和全局规划；楼梯过渡与定位稳定性也由同一文件驱动。不要直接编辑 106 的 `/etc/m20pro-edge-scan-106.env`、Nav2 参数占位符或定位参数占位符。整狗部署会比较上位机、104 和 106 的配置哈希，不一致即失败；任务运行中不支持热更新。
