...
遥控弹层用一个姿态按钮在“起立/趴下”之间切换，目标动作优先根据原厂 `/MOTION_STATE` 状态决定。起立/趴下会先结束网页接管、锁定速度，再通过原厂 `Type=2/Command=22` 下发 `MotionParam=1/4`；软急停下发 `MotionParam=2`，同时停止任务并保持锁定。中央方块按钮仅发送零速并保持接管，与改变本体状态的软急停语义不同。原厂手柄不经过本项目仲裁器，不能与网页遥控同时操作。
