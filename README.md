...
现场允许调整的点云投影、跨层路线元数据、Nav2、遥控和定位稳定性参数只有一个源文件：
...
当前网页一次显示一张单层地图。跨楼层点位、有向路线和任务编排数据仍保留，但 2026-07-19 引入的楼梯三维感知、扫描选择器和旧爬楼执行入口已撤除。新爬楼方案接入前，`floor_manager` 会在机器狗开始跨层运动前返回 `stair_execution_retired`；单层导航、自检、前端和录包统一直接使用 106 的 `/scan`。地图可在地图浮层删除；当前地图、任务执行期间和跨楼层路线引用的地图受保护。删除普通地图会同步清理 104 上的点位、任务和建图会话引用，不会删除独立保存的雷达历史结果，也不会自动删除 106 原厂地图包。需要清理 106 时必须先确认 `active` 不指向目标包，再精确删除对应原厂目录。
...
- 爬楼梯点和出楼梯点当前只作为跨层路线语义数据保存；旧 `stair_up/stair_down` 执行入口已停用，点位名称或 `Gait` 值都不会绕过 `stair_execution_retired` 保护。
...
遥控弹层用一个姿态按钮在“起立/趴下”之间切换。`m20pro_tcp_bridge` 按开发手册发送心跳并接收原厂 2Hz `Type=1002/Command=6` 主动基础状态，从 `BasicStatus.MotionState` 生成唯一项目姿态话题；趴下/阻尼/空闲状态显示“起立”，站立/标准运动/敏捷运动状态显示“趴下”，状态缺失或超过 3 秒时显示“姿态未知”并禁止误下发。起立/趴下会先结束网页接管、锁定速度，再通过原厂 `Type=2/Command=22` 下发 `MotionParam=1/4`；软急停下发 `MotionParam=2`，同时停止任务并保持锁定。中央方块按钮仅发送零速并保持接管，与改变本体状态的软急停语义不同。原厂手柄不经过本项目仲裁器，不能与网页遥控同时操作。

状态栏电量可展开查看电压、电流、温度、电池 MOS/保护状态和电流方向判断；103 原厂当前未提供可用的独立充电状态查询接口，因此不伪造“充电中”状态。“一键充电”先由 Nav2 导航到当前地图唯一充电点，再由 104 通过 TCP 桥向 103 下发原厂 `Type=1003/Command=1`、强制 `PointInfo=3` 的充电导航命令；只有收到原厂明确回执后任务才结束。没有充电点或存在多个充电点时，系统拒绝执行，不猜测充电桩位置。充电请求和回执分别记录在 `/m20pro/charge_command`、`/m20pro_tcp_bridge/charge_result`。
...
3. 跨楼层只做数据静态验收：确认每层只有一张路线正式地图、上下行有向路线齐全、四点均来自对应地图。新爬楼方案完成前不执行真实跨层运动，并确认系统返回 `stair_execution_retired`。

同楼层真导航必须录包。出现原地转圈、明显偏航、贴障碍物、路径穿墙、地图和当前位置明显不匹配时，立即点击网页停止任务或使用手柄急停。
