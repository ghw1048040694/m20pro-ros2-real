...
当前网页一次显示一张单层地图。跨楼层任务通过点位携带的楼层字段和 `/m20pro/floor_goal` 交给 `floor_manager` 处理。普通 104 归档地图可在地图浮层删除；当前地图、项目内置地图和任务执行期间受保护，删除普通地图会同步清理其点位、任务和建图会话引用，但不会删除独立保存的雷达历史结果。
