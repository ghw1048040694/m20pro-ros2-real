...
- 爬楼梯点和出楼梯点作为有向跨层路线的入口与出口；步态与速度由唯一 `stair_executor` 按路线方向统一下发，不由点位名称触发旧链路。
...
3. 最小跨层验收：先只配置两层、一条上行路线和每层一个近距离任务点；用 `104_start_real_move.sh` 启动，确认入口、上楼、平台切图重定位、出口和目标点的完整顺序。上行单程通过后再配置反向路线测试下楼，下楼使用后退。
