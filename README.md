...
遥控弹层还提供起立、趴下和软急停：起立/趴下会先结束网页接管、锁定速度，再通过原厂 `Type=2/Command=22` 下发 `MotionParam=1/4`；软急停下发 `MotionParam=2`，同时停止任务并保持锁定。原厂手柄不经过本项目仲裁器，不能与网页遥控同时操作。

状态栏电量可展开查看电压、电流、温度、原厂充电状态和运动状态；“一键充电”复用当前地图唯一充电点（`PointInfo=3`）启动任务。没有充电点或存在多个充电点时，系统拒绝执行，不猜测充电桩位置。
