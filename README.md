...
当前网页一次显示一张单层地图。跨楼层任务通过点位携带的楼层字段和 `/m20pro/floor_goal` 交给 `floor_manager` 处理。地图可在地图浮层删除；当前地图和任务执行期间受保护，普通 104 归档地图会删除受管文件，F19/F20/F21 等项目资产按普通业务地图处理，可从业务地图库移除且后续部署不会重新显示，但保留仓库只读源文件。删除地图会同步清理其点位、任务和建图会话引用，不会删除独立保存的雷达历史结果。
